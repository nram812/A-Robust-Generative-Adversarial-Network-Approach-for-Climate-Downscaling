...
def compute_quantiles(df,quantiles, period):
    ...
    quantiled_rain = df.quantile(q = quantiles, dim =["time","member"], skipna =True)
...
for gcm in stacked_X.GCM.values:
    ...
    output_hist = xr.concat([predict_parallel_resid(gan, unet,
                                   ...
                                   output_shape.sel(time = historical_period), 64, orog.values, he.values, vegt.values, model_type='GAN') for i in range(3)],
                            dim ="member")
    ...
    output_future = xr.concat([predict_parallel_resid(gan, unet,
                                         ...
                                         vegt.values, model_type='GAN') for i in range(3)], dim ="member")
