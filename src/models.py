...
def unet_linear(input_size, resize_output, num_filters, kernel_size, num_channels, num_classes, resize=True):
    ...
    **Example Usage:**
    ...
    # add some noise within the GAN framework
    x_output = res_block_initial(concat_noise, [num_filters[-1]], 3, [1, 1], "input_layer", sym_padding =False)
    ...
    output = res_block_initial(output, [64], 3, [1, 1], "output_convbas1", sym_padding =False)
    output = res_block_initial(output, [32], 3, [1, 1], "output_conv", sym_padding =False)
    output = tf.image.resize(output, (resize_output[0], resize_output[1]), method=tf.image.ResizeMethod.BILINEAR)

    #output = SymmetricPadding2D(padding=[2, 2])(output)
    output = tf.keras.layers.Conv2D(32,
                                    5,
                                    strides=1,
                                    padding='same',
                                    name='custom_precip_layer', activation=tf.keras.layers.LeakyReLU(0.3))(output)
    #output = SymmetricPadding2D(padding=[3, 3])(output)
    output = tf.keras.layers.Conv2D(16,
                                    7,
                                    strides=1,
                                    padding='same',
...
def unet_linear_v2(input_size, resize_output, num_filters, kernel_size, num_channels, num_classes, resize=True,
    ...
    x = tf.keras.layers.Concatenate(-1)([x1, x])
    # decode
    x = up_block(x, temp3, kernel_size=5, filters = num_filters[2], i =0)
    x = up_block(x, temp2, kernel_size=5, filters = num_filters[1], i =2)
