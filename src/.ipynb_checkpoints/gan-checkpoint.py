...
class WGAN_Cascaded_Residual_IP_CC_pres(keras.Model):
    ...
    def g_step(self, real_images, real_images_future, random_latent_vectors, average, average_future, orog_vector, he_vector, vegt_vector):
        ...
        with tf.GradientTape() as tape:
            ...
            g_loss = 0.5 * (adv_loss + adv_loss_future + gamma_loss_func + mae_future) + 0.5 * self.intensity_weight * (tf.reduce_mean(
...
class WGAN_Cascaded_IP(keras.Model):
    ...
    def __init__(self, discriminator, generator, latent_dim,
                 discriminator_extra_steps=3, gp_weight=10.0, ad_loss_factor=1e-3,
                 latent_loss=5e-2, orog=None, he=None,
                 vegt=None, unet=None, train_unet=True, intensity_weight = 1, average_intensity_weight =0.0):
    ...
    def train_step(self, real_images):
        ...
        with tf.GradientTape() as tape:
            ...
            generated_images_v2 = [tf.expand_dims(self.generator(
                ...
                training=True), axis =0) for i in range(8)]
